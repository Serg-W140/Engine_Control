#include "main.h"   // Базовая библиотека STM32 HAL от STMicroelectronics
#include "u8g2.h"   // Графическая библиотека для OLED-экранов 1.3" (SH1106)
#include <stdio.h>  // Для функции sprintf (форматирование чисел в текст)
...
#define DWELL_POINTS  4   // Количество точек для таблицы накопления зажигания
...
extern TIM_HandleTypeDef htim1;  // Таймер №1 для ШИМ форсунки
extern TIM_HandleTypeDef htim2;  // Таймер №2 для точных микросекундных фаз зажигания
...
// Таблица времени накопления катушки ВАЗ от напряжения сети
const float DWELL_VOLT_AXIS[DWELL_POINTS] = {10.0f, 12.0f, 14.0f, 16.0f}; // Вольты
const float DWELL_TIME_AXIS[DWELL_POINTS] = {5.00f, 3.80f, 3.00f, 2.40f}; // Миллисекунды накопления
...
// Новая функция: плавно вычисляет время накопления катушки под текущие Вольты сети
...
// ============================================================================
// ГЛАВНАЯ ФУНКЦИЯ ПРОГРАММЫ ЭБУ (НАЧАЛО ИСПОЛНЕНИЯ КОДА)
// ============================================================================
int main(void)
{
  // Автоматические стартовые настройки железа, которые генерирует CubeMX
  HAL_Init();           // Инициализация слоя HAL (базовые таймеры и задержки)
  ...
  MX_ADC1_Init();      // Настройка и запуск АЦП №1 (опрос датчиков)
  MX_TIM1_Init();      // Настройка Таймера №1 (ШИМ-генератор для форсунки)
  MX_I2C1_Init();      // Настройка шины I2C №1 для связи с дисплеем и памятью

  // 1. АППАРАТНЫЙ СТАРТ ДАТЧИКОВ (Запускаем связку АЦП + Робот DMA)
  // Робот DMA сам без участия процессора по кругу пишет данные 7 датчиков в буфер adc_raw_buffer
  HAL_ADC_Start_DMA(&hadc1, (uint32_t*)adc_raw_buffer, 7);

  // 2. АППАРАТНЫЙ СТАРТ ФОРСУНКИ (Запускаем ШИМ на Таймере 1, Канале 1. Ножка PA8 готова)
  ...
  // Строковые текстовые буферы для рисования параметров на приборной панели
  char str_rpm[16]; char str_fuel[16]; char str_uoz[16];
  int main_injector_fault = 0; // Переменная статуса форсунки (для теста защит)
  ...
  while (1)
  {
    ...
    // Переводим попугаи АЦП в реальное напряжение АКБ с учетом резисторного делителя (1 кОм и 1.6 кОм)
    battery_voltage = (float)adc_bat * (3.3f / 4095.0f) * (2.6f / 1.6f); 

    // Проверяем электрическую исправность датчиков (защита от обрыва и КЗ)
    ...
    // АВТО-ЛОГГЕР: Выжигаем во внешнюю EEPROM по I2C только если сбой СВЕЖИЙ
    ...
    if (temperature >= 70 && tps < 80) {
        ...
        if (dk_voltage < 0.4f) {
            o2_trim += 0.01f; if (o2_trim > 1.20f) o2_trim = 1.20f; // Бедно -> обогащаем смесь
        } else if (dk_voltage > 0.5f) {
            o2_trim -= 0.01f; if (o2_trim < 0.80f) o2_trim = 0.80f; // Богато -> обеняем смесь
        }
    } else {
    ...
    // Формула Идельного Газа: Базовое время впрыска привязано к производительности форсунки
    ...
    float dead_time = get_interpolated_dead_time(battery_voltage); // Плавный лаг форсунки от вольтажа
    ...
    // Логика дублирования форсунок при отказах (Аппаратный резерв!)
    ...
    // Рассчитываем точное время накопления под текущий вольтаж АКБ (от 2.4 до 5.0 мс)
    float current_dwell_ms = get_interpolated_dwell_time(battery_voltage); 
    
    // Переводим миллисекунды накопления в ГРАДУСЫ поворота коленвала для текущих оборотов
    float dwell_degrees = (current_dwell_ms * rpm * 6.0f) / 1000.0f; 

    // Находим угол, на котором нужно НАЧАТЬ подавать ток в катушку (Угол Заряда)
    float charge_angle = uoz + dwell_degrees; 

    // Проверяем фазу коленвала по прерываниям ДПКВ
    if (current_cylinder_pair == 14) {
        // --- КАТУШКА А (Цилиндры 1 и 4) ---
        // В реальном STM32 это делает аппаратное сравнение таймера (Compare), 
        // но логика процесса жестко привязана к расчетным углам:
        
        // 1. Включаем ток накопления на ножке PB0 ровно на угле charge_angle
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, GPIO_PIN_SET); 
        
        // 2. Ждем разницу градусов (наше время накопления), переведенную в микросекунды
        HAL_Delay_us((uint32_t)(current_dwell_ms * 1000.0f)); 
        
        // 3. Мгновенно обрываем ток строго на угле УОЗ (uoz). ИСКРА А!
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, GPIO_PIN_RESET); 
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, GPIO_PIN_RESET); // Катушка Б спит
    } 
    else if (current_cylinder_pair == 23) {
        // --- КАТУШКА Б (Цилиндры 2 и 3) ---
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, GPIO_PIN_SET);   // Накопление Б (ножка PB1)
        HAL_Delay_us((uint32_t)(current_dwell_ms * 1000.0f)); 
